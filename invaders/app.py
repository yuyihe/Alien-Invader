...
or the models module. If you are unsure about where a new class should go, post
...
class Invaders(GameApp):
    ...
    LIST MORE ATTRIBUTES (AND THEIR INVARIANTS) HERE IF NECESSARY
        _speed: float, the speed of the aliens marching across the screen in this game
        _life: int, the number of life that the ship has in this game
        _lifetext: the displayed text that says how many lives the player currenty
        has in this game
        _score: int, the score that the player gets in this game
        _scoretext: the displayed text that says what score the player currenty
        has in this game
    """
    ...
    def start(self):
        ...
        Invariants: Attribute _text is only None if _state is STATE_ACTIVE.
        """
        # IMPLEMENT ME
        ...
        self._life = SHIP_LIVES
        self._score = 0
        self._lifetext = GLabel(left=0,top=GAME_HEIGHT,
        text='Lives: '+str(self._life),font_name='RetroGame')
        self._scoretext = GLabel(right=GAME_WIDTH,top=GAME_HEIGHT,
    ...
    def update(self,dt):
        ...
        if self._state == STATE_INACTIVE:
            ...
            self._wave = Wave(self._speed,self._life,self._score)
            ...
            self._life = self._wave.getLife()
            self._lifetext.text = 'Lives: '+str(self._life)
            self._text = GLabel(x=GAME_WIDTH/2,y=GAME_HEIGHT/2,
            text=str(self._life)+' lives left. Press "s" to resume',
            font_size='40',font_name='arcade')
            self._key()
    ...
    def _activeStateHelper(self,dt):
        ...
        """
        self._pause()
        self._wave.update(self._shipDirection(),dt,self)
        if self._wave.getState() == STATE_PAUSED:
            self._state = STATE_PAUSED
        elif self._wave.getFail():
            self._state = STATE_COMPLETE
            self._text = GLabel(x=GAME_WIDTH/2,y=GAME_HEIGHT/2,
            text='You lost. Press "s" to start new game',font_size='40',
            font_name='arcade')
            self._key()
        elif self._wave.getAllKilled():
            self._state = STATE_COMPLETE
            self._text = GLabel(x=GAME_WIDTH/2,y=GAME_HEIGHT/2,
            text='You won! Press "s" to start a new round',font_size='40',
            font_name='arcade')
            self._key()
    ...
    def _key(self):
        ...
        if self.input.is_key_down("s"):
            ...
            if self._state == STATE_INACTIVE:
                ...
                self._wave.setState(STATE_ACTIVE)
                self._state = STATE_ACTIVE
            elif self._state == STATE_COMPLETE:
                if self._wave.getAllKilled():
                    self._speed = self._speed*ALIEN_FASTER
                    self._life = self._wave.getLife()
                    self._lifetext.text = 'Lives: '+str(self._life)
                    self._score = self._wave.getScore()
                    self._scoretext.text = 'Score: '+str(self._score)
    ...
    def _shipDirection(self):
        """
        Returns the direction that player wants the ship to move

        Detect when player press down 'left' or 'right' and move the ship
        accordingly
